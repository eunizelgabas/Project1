    ...
    <div>
        <div class="overflow-y-auto overflow-x-hidden flex-grow">
            <ul class="flex flex-col py-2 space-y-1">
                ...
                <li :class="{'bg-blue-100 font-bold text-gray-800  border-transparent border-indigo-900': route().current('create.index') }" v-if="$page.props.auth.permissions.includes('manage-user')">
                    <Link :href="route('user.index')" class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-blue-100  hover:text-gray-800 border-l-4 border-transparent hover:border-indigo-900 pr-6">
                        ...
                        <span class="ml-2 text-m tracking-wide truncate" v-show="!collapse">Create User</span>
                    </Link>
                </li>
